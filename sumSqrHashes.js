...
function sumSqrHashes(inStr, findStr) {
  const result = { collision: 0, index: [] };
  let findHash = 0;
  let currentHash = 0;
  for (let i = 0; i < findStr.length; i++) {
    findHash += findStr.charCodeAt(i) ** 2;
    currentHash += inStr.charCodeAt(i) ** 2;
  }
  ...
  for (let i = 1; i <= inStr.length - findStr.length; i++) {
    currentHash +=
      inStr.charCodeAt(i + findStr.length - 1) ** 2 -
      inStr.charCodeAt(i - 1) ** 2;
    if (findHash === currentHash) {
      result.collision++;
...
exports.sumSqrHashes = sumSqrHashes;
